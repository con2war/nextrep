...
export default function DailyWorkout() {
  const [step, setStep] = useState<
    "goal" | "equipment" | "preferences" | "generating" | "workout"
  >("goal");
  ...
  // Dummy data for goals, equipment, muscle groups, and time limits.
  const goals = [
    {
      id: "strength",
      ...
      icon: "💪",
      description:
        "Focus on heavy compound lifts, low reps, and longer rest times to increase raw strength.",
    },
    {
      id: "hypertrophy",
      ...
      icon: "🏋️‍♂️",
      description:
        "Optimized for muscle size, using moderate reps (8-12), controlled tempo, and progressive overload.",
    },
    {
      id: "powerlifting",
      label: "Powerlifting",
      icon: "🏋️",
      description:
        "Train for maximum lifts in squat, bench press, and deadlift with heavy loads and low reps (1-5).",
    },
    {
      id: "yoga",
      ...
      icon: "🧘‍♂️",
      description:
        "Focus on mobility, flexibility, and relaxation through dynamic and static stretching.",
    },
    {
      id: "conditioning",
      ...
      icon: "⚡",
      description:
        "Improve cardiovascular fitness, speed, and agility with high-intensity movements and circuits.",
    },
    {
      id: "endurance",
      ...
      icon: "🏃‍♂️",
      description:
        "Train with higher reps (12-20), short rest times, and aerobic conditioning for long-term stamina.",
    },
    {
      id: "fat_loss",
      ...
      icon: "🔥",
      description:
        "Utilizes high-intensity interval training (HIIT), supersets, and circuits to maximize calorie burn and metabolism.",
    },
    {
      id: "mobility",
      ...
      icon: "🤸‍♂️",
      description:
        "Focus on joint health, mobility drills, and dynamic stretching to improve movement quality.",
    },
    {
      id: "functional",
      ...
      icon: "⚡",
      description:
        "Train for real-world movements using kettlebells, bodyweight exercises, and multi-joint movements.",
    },
    {
      id: "athletic",
      ...
      icon: "🏆",
      description:
        "Enhance speed, agility, strength, and power for sport-specific training.",
    },
  ];
  ...
  const equipmentCategories = [
    ...
    },
    {
      ...
      items: [{ id: "bodyweight", label: "Bodyweight Only" }],
    },
  ];
  ...
  const handleGenerateWorkout = async () => {
    ...
    try {
      const selectedSplit = selectedMuscleGroups.find((group) =>
        exerciseSplits.includes(group)
      );
      const specificMuscles = selectedMuscleGroups.filter(
        (group) => !exerciseSplits.includes(group)
      );
  ...
  if (isWorkoutStarted && workout) {
    return (
      <WorkoutSession workout={workout} onComplete={handleWorkoutComplete} />
    );
  }

  if (workoutSummary) {
    const totalStats = {
      sets: Object.values(workoutSummary.exercises)
        .flat()
        .reduce((acc, ex) => acc + (ex.completed ? 1 : 0), 0),
      reps: Object.values(workoutSummary.exercises)
        .flat()
        .reduce((acc, ex) => acc + ((ex.completed ? 1 : 0) * (ex.reps || 0)), 0),
      weight: Object.values(workoutSummary.exercises)
        .flat()
        .reduce((acc, ex) => acc + (ex.weight || 0), 0),
      volume: Object.values(workoutSummary.exercises)
        .flat()
        .reduce(
          (acc, ex) =>
            acc + ((ex.weight || 0) * (ex.reps || 0) * (ex.completed ? 1 : 0)),
          0
        ),
    };
      ...
      <div className="min-h-screen max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="space-y-4">
            {Object.entries(workoutSummary.exercises).map(
              ([section, exercises]) => (
            ...
            <div key={section} className="mb-8">
              ...
              <div className="space-y-4">
                {Array.isArray(generatedWorkout?.[section as keyof WorkoutData]) &&
                  (generatedWorkout[section as keyof WorkoutData] as Exercise[]).map(
